...
// export default {
    // setup(){
const fGArr = [
       {branch:"Delwathon group", fee_group:"School fee", fee_breakdown:"Schoo fee"},
       {branch:"Delwathon college", fee_group:"College fee", fee_breakdown:"End of the year Fee"}
]
const state = reactive({
    ...
    const search = (searchText) =>{
        ...
        const searched = fGArr.filter(data =>(
            data.branch.toLocaleLowerCase().includes(tolower) ||
            data.fee_group.toLocaleLowerCase().includes(tolower)
        ))
        state.data = searched
...
<template>
    <div class="container md:container md:p-4 p-3">
         <div class="flex  justify-between items-center">
            <h2 class="font-medium text-sm uppercase mt-4 md:mt-0">FEE GROUP</h2>
            <div class="[font-size:12px] mt-4 md:mt-0">Fee Group</div>
        </div>
        <!--  -->
                        ...
                        <div class=" md:-p4 p-4">
                            ...
                            <div class="grid grid-col-1 md:grid-cols-1 mx-auto mt-2 md:p-3 p-2">
                               ...
                               <div class="w-full mt-4">
                                    <label for="" class="mb-4 text-[#4F4F4F] dark:text-inherit after:content-['*'] after:ml-0.5 after:text-red-500">Fee group</label>
                                    <input type="text" class="app-input w-full mt-1" placeholder="Enter a fee group name">
                               </div>
                               <div class="w-full mt-4">
                                    <label for="" class="mb-4 text-[#4F4F4F] dark:text-inherit after:content-['*'] after:ml-0.5 after:text-red-500">Breakdown</label>
                                   <select name="" class="app-input w-full" id="">
                                        <option value="" selected>Choose...</option>
                                        <option value="1">Delwathon Group</option>
                                    </select>
